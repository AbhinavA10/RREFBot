// TODO: make robot move up after scanning
...
task main()
{
	...
	if(hasYelledEnough){
		...
		recognizeDigits();
		/*
		//outputting the raw data of the scan
		outputMatrix1();
		wait1Msec(3000);
		outputMatrix2();
		wait1Msec(3000);
		...
		*/
		...
		//http://help.robotc.net/WebHelpMindstorms/index.htm#Resources/topics/
		//LEGO_EV3/ROBOTC/Sounds/playSoundFile.htm
		setSoundVolume(100);//Sets the sound volume of the EV3 speaker to number
...
void moveRobotDown(int distanceDown, bool down){
	nMotorEncoder[MOTOR_AXIAL] = 0;
	if(down){
		motor[MOTOR_AXIAL] = -AXIAL_MOTOR_SPEED;
...
void placePen (bool setPen)
{
	...
	if (setPen == 0)
	{
		...
		while (nMotorEncoder [MOTOR_PEN] < ANGLE_PEN)
		{}
	}
	else
	{
...
void scanMatrix(){
	bool isBlack = false;
	for(int row=0; row < ROW_NUM_TOTAL; row++){
		for(int col=0; col < COL_NUM_TOTAL; col++){
		...
		// on the track (left side)
		nextLineDown(); // move the robot down 1 'cell' to scan the next line
...
void recognizeDigits(){
	...
	for(int index=0; index<ROW_NUM; index++){
		...
		// finding the most similar predefined matrix
			...
			matrix[0][0] = index; // 'recognizing' the scanned matrix as a digit
		}
